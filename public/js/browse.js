...
search.addWidget(
    instantsearch.widgets.searchBox({
        ...
        placeholder: 'type pipeline name or keyword...'
    }));
...
search.addWidget(
    instantsearch.widgets.hits({
        ...
        templates: {
            empty: 'No results',
            item: function(data) {
                var pipeline_name = data._highlightResult.pipeline_name.value;
                
                var pipeline_summary = data._highlightResult.pipeline_summary.value;
                var pipeline_keywords ='<span style="font-size:85%;">';
		        var idx_kw=1;
                data._highlightResult.pipeline_keywords.forEach(function(entry){
		              if (idx_kw>1) { 
                          pipeline_keywords += " - " 
                      }
                      pipeline_keywords += "<i>"+entry.value + "</i>";
		              idx_kw+=1;
                });
